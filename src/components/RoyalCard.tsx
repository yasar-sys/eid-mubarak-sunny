...
const RoyalCard = ({ onReveal, cardData }: { onReveal: () => void; cardData: CardData }) => {
  ...
  const downloadCard = async (e: React.MouseEvent) => {
    ...
    try {
      // Capture the card. We use a high pixel ratio for better quality.
      const dataUrl = await toPng(cardRef.current, {
        cacheBust: true,
        width: cardRef.current.offsetWidth,
        height: cardRef.current.offsetHeight,
        style: {
          transform: 'none', // Remove any temporary transforms
          borderRadius: '16px'
        }
      });
      const link = document.createElement('a');
      link.download = `eid-card-from-${cardData.senderName.toLowerCase().replace(/\s+/g, '-')}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
        ...
        >
          ...
          >
            ...
            <motion.button
              ...
              className="mt-3 flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] font-cinzel tracking-widest uppercase transition-all duration-300"
